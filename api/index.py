"""
Snack Attack! - Vercel Serverless Entry Point & Flask Web Application
"""
import os
import json
...
# Resolve templates and static directories with fallback
def _resolve_dir(folder_name: str) -> str:
    candidates = [
        os.path.join(CURRENT_DIR, folder_name),
        os.path.join(PARENT_DIR, folder_name),
        os.path.join(os.getcwd(), folder_name),
        os.path.join(os.getcwd(), "api", folder_name)
    ]
    for c in candidates:
        if os.path.isdir(c):
            return c
...
TEMPLATE_DIR = _resolve_dir("templates")
STATIC_DIR = _resolve_dir("static")

# Initialize Flask App with resolved directories
app = Flask(__name__, template_folder=TEMPLATE_DIR, static_folder=STATIC_DIR)

# Expose WSGI callable aliases for Vercel Serverless Functions
handler = app
application = app
...
def get_read_path(filename: str) -> str:
    """Return file path for reading, checking /tmp, local api/, and root dir."""
    if IS_SERVERLESS:
        tmp_file = os.path.join("/tmp", filename)
        if os.path.exists(tmp_file):
            return tmp_file
    for c in [os.path.join(CURRENT_DIR, filename), os.path.join(PARENT_DIR, filename), os.path.join(os.getcwd(), filename)]:
        if os.path.exists(c):
            return c
